...
function PreviewNavigationBar(): JSX.Element {
  const { navigationItemStore } = useContext(DataContext);

  const roots = useMemo(() => navigationItemStore.getRoots(), [navigationItemStore]);
  ...
  return (
    <nav className="navbar navbar-light bg-faded navbar-toggleable-md mb-4">
      <div className="navbar-brand">Preview</div>

      <div className="collapse navbar-collapse" id="preview-navbar-collapse-1">
        <ul className="nav navbar-nav mr-auto">{renderNavigationItems()}</ul>
      </div>
    </nav>
  );
}
